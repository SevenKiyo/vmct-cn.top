const sidebar = {
  "/modpacks": [
    { text: "返回首頁", link: "/tw/index" },
    { text: "地圖漢化", link: "/tw/map" },
    { text: "中文更新檔安裝教程", link: "modpacks" },
    { text: "用戶協定", link: "/tw/agreement" },
    { text: "隱私政策", link: "/tw/privacy" },
    {
      text: "模組包漢化",
      collapsed: false,
      items: [
        { text: "破碎城", link: "/modpacks/fractured-opolis" },
        { text: "FTB Skies2: Aero", link: "/modpacks/skies2-aero" },
        { text: "FTB石头世界4", link: "/modpacks/sb4/" },
        { text: "謎之天空", link: "/modpacks/enigmatic-skies" },
        { text: "吸血鬼的反擊", link: "/modpacks/vampires-strike-back" },
        { text: "Submerged 2", link: "/modpacks/submerged-2" },
        {
          text: "Ozone Skyblock Reborn 2",
          link: "/modpacks/ozone-skyblock-reborn-2",
        },
        { text: "科技城3", link: "/modpacks/techopolis3" },
        { text: "Roguelike冒險與地牢（RAD1）", link: "/modpacks/rad/" },
        { text: "Roguelike冒險與地牢2（RAD2）", link: "/modpacks/rad2" },
        { text: "永恆的MC 2", link: "/modpacks/mce2" },
        { text: "閾限工業", link: "/modpacks/liminal-industries" },
        { text: "星河邊疆", link: "/modpacks/cosmic-frontiers" },
        { text: "FTB Evolution", link: "/modpacks/ftbe" },
        { text: "Craftoria", link: "/modpacks/craftoria" },
        { text: "異域征途", link: "/modpacks/prodigium-reforged" },
        { text: "亡者世界", link: "/modpacks/deceasedcraft" },
        { text: "FTB Skies 2", link: "/modpacks/skies-2" },
        { text: "FTB Neotech", link: "/modpacks/neotech" },
        { text: "FTB Oceanblock 2", link: "/modpacks/oceanblock2" },
        { text: "FTB石頭世界3", link: "/modpacks/sb3" },
        { text: "Integrated MC", link: "/modpacks/imc" },
        { text: "Bliss", link: "/modpacks/bliss" },
        { text: "Chunk Block", link: "/modpacks/chunk" },
        { text: "探索者前夜-凋靈風暴", link: "/modpacks/explorers-eve" },
        { text: "FTB天空奧德賽", link: "/modpacks/odyssey" },
        { text: "FTB建築師天堂2", link: "/modpacks/paradise2" },
        { text: "FTB Presents Direwolf20 1.19", link: "/modpacks/wolf" },
        { text: "Muskteer", link: "/modpacks/muskteer" },
        { text: "Prey", link: "/modpacks/prey" },
        { text: "The Decursio Project Expert", link: "/modpacks/decursio" },
        { text: "奧術學院", link: "/modpacks/arcanum" },
        { text: "寶藏獵人3", link: "/modpacks/vault" },
        { text: "冰雪之域", link: "/modpacks/frozenopolis" },
        { text: "春曉之巫", link: "/modpacks/cottage" },
        { text: "曲奇製造者", link: "/modpacks/cookiecrafter" },
        { text: "洞穴城", link: "/modpacks/cave" },
        { text: "等價交換工程2", link: "/modpacks/architect2" },
        { text: "地獄起源", link: "/modpacks/infernal" },
        { text: "幻想領域", link: "/modpacks/fantasy" },
        { text: "黑石世界", link: "/modpacks/black" },
        { text: "機械動力：通用機械化", link: "/modpacks/create-mekanized" },
        { text: "機械動力：星辰", link: "/modpacks/create" },
        { text: "渴望科技2", link: "/modpacks/hungry" },
        { text: "農業森友會5", link: "/modpacks/fc5" },
        { text: "噴氣背包貓", link: "/modpacks/jetpack" },
        { text: "破曉之界", link: "/modpacks/dawn" },
        { text: "餵養奇點生存", link: "/modpacks/feed" },
        { text: "無限協會", link: "/modpacks/if" },
        { text: "重生之夜", link: "/modpacks/rotn" },
        { text: "天空工廠5", link: "/modpacks/sf5" },
        { text: "星辰工廠", link: "/modpacks/star" },
        { text: "永恆的MC", link: "/modpacks/mce" },
        { text: "中世紀的MC 1.19 Fabric", link: "/modpacks/mmc" },
        { text: "核荒地", link: "/modpacks/nuclear" },
        { text: "卓越2", link: "/modpacks/prominence2" },
        { text: "Enigmatica 9: Expert E9E", link: "/modpacks/e9e" },
      ],
    },
  ],
  "/modpacks/fc5-wiki": [
    { text: "農業森友會5 wiki", link: "index" },
    {
      text: "wiki",
      collapsed: false,
      items: [
        { text: "農業森友會5", link: "/modpacks/fc5-wiki" },
        {
          text: "Farming Crossing Craving Mechanic",
          link: "/modpacks/fc5-wiki/farming",
        },
        {
          text: "Modpack Exclusive Features",
          link: "/modpacks/fc5-wiki/features",
        },
        {
          text: "Performance Improvements",
          link: "/modpacks/fc5-wiki/performance",
        },
        { text: "Quest Overview", link: "/modpacks/fc5-wiki/quest" },
        { text: "Miles Tickets", link: "/modpacks/fc5-wiki/tickets" },
        {
          text: "Suggest Translation Improvements",
          link: "/modpacks/fc5-wiki/translation",
        },
        { text: "Tutorials", link: "/modpacks/fc5-wiki/tutorials" },
        { text: "Modpack Tweaks", link: "/modpacks/fc5-wiki/tweaks" },
        {
          text: "Villagers available to move in",
          link: "/modpacks/fc5-wiki/villagers",
        },
      ],
    },
  ],
  "/tw/map": [
    { text: "返回首頁", link: "/tw/index" },
    { text: "模組包漢化", link: "modpacks" },
    { text: "漢化準則", link: "https://docs.vmct-cn.top/rule/" },
    { text: "雙十一事變", link: "what" },
    { text: "用戶協定", link: "/tw/agreement" },
    { text: "隱私政策", link: "/tw/privacy" },
    {
      text: "地圖漢化",
      collapsed: false,
      items: [
        { text: "最終悖論", link: "/tw/map/final" },
        { text: "神界隕落", link: "/tw/map/divinity" },
        { text: "貝維爾小鎮", link: "/tw/map/bayville" },
        { text: "死亡競速2", link: "/tw/map/deathrunner" },
        { text: "侦探故事集", link: "/tw/map/detective-stories" },
        { text: "Evergrowth", link: "/tw/map/evergrowth" },
        { text: "玩具熊的五夜後宮4", link: "/tw/map/fnaf-4-miramappa" },
        { text: "Lumina Complex", link: "/tw/map/lumina" },
        { text: "樞機迷城", link: "/tw/map/machine" },
        { text: "狂亂之境4", link: "/tw/map/rc4" },
        { text: "太陽守衛者2：擱淺", link: "/tw/map/sun-guardian2" },
        { text: "Tales of Glarthford", link: "/tw/map/tales-of-glarthford" },
        { text: "露臺3：碎裂", link: "/map/the-terrace-3-shattered" },
        { text: "藝之神殿", link: "/tw/map/tota" },
        { text: "Wii派對：棋盤遊戲島", link: "/tw/map/wii-party" },
        { text: "Would You Rather", link: "/tw/map/wyr" },
        { text: "Yoodexy's Mod", link: "/tw/map/yoodexy" },
      ],
    },
  ],
};

export default sidebar;
